...
LSS should understand the IT 140 development workflow well enough to help students **use the tools as part of learning** and to recognize when a problem requires technical support.

LSS are not expected to perform deep Git recovery, operating-system repair, or course automation maintenance.
...
4. use the shared [GitHub Workflow](../shared/github-workflow.md) for common repository concepts.
...
<!-- screenshot placeholder; show the GitHub owner/repository breadcrumb on a GC-STEM template and on a private student repository so LSS can distinguish the two -->

A student can appear to have "lost" work simply because the wrong repository copy or local folder is open.
...
- why a missing local folder may be restored from an existing private GitHub repository; and
- where repository instructions, tests, and supporting files are located.

Use the canonical [GitHub Workflow](../shared/github-workflow.md) rather than duplicating procedural details here.

## Private Repository Access

A student's assignment/project repository is private.

If direct LSS review of the repository is appropriate, the student may grant collaborator access through the approved GitHub workflow.
...
- avoid changing or committing to the repository unless an approved support process specifically requires it; and
- keep the student engaged in explaining and changing their own work.

## Useful Read-Only Information

When trying to understand repository state, useful non-secret information can include:
...
See [Supported Environments](../shared/supported-environments.md) for the canonical environment facts.
...
- template creation or cloning fails as documented;
- repository recovery risks losing student work; or
- Verify reports `NOT COMPLIANT` or another environment failure.

Use [Service Desk Triage](../service-desk/triage.md) as the technical-support reference.

## Do Not Use "Start Over" as the First Fix

A repository problem can involve multiple copies of the student's work.

Before any destructive action, preserve student work and use the documented repository recovery path.
...
Technical recovery beyond ordinary documented steps should move to the Service Desk.
